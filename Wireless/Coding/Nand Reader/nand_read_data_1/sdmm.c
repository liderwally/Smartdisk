...
#define DO_INIT() portModeOutput()			/* Initialize port for MMC DO as input */
#define DO			(PINB &	0x01)	/* Test for MMC DO ('H':true, 'L':false) */

#define DI_INIT()	portModeInput()	/* Initialize port for MMC DI as output */
...
#define CK_H()		digitalWrite(NAND_W, HIGH);	/* Set MMC SCLK "high" */
#define	CK_L()		digitalWrite(NAND_W, LOW);	/* Set MMC SCLK "low" */

#define CS_INIT()	DDRB  |= 0x08	/* Initialize port for MMC CS as output */
#define	CS_H()		nandEnable(LOW)	/* Set MMC CS "high" */
#define CS_L()		nandEnable(HIGH)	/* Set MMC CS "low" */
