...
long int onWriteSD(uint32_t lba, uint32_t offset, unsigned char *buffer, uint32_t bufsize) {

  bool err = true;
  for (int i = 0; i < bufsize / SD_MMC.sectorSize(); i++) {
    err &= SD_MMC.writeRAW((uint8_t *)(buffer + ( i*SD_MMC.sectorSize() )), lba + i);
  }
  //DBG_OUTPUT_PORT.printf("MSC WRITE: lba: %u, offset: %u, bufsize: %u\n", lba, (unsigned int)offset, bufsize);
  return (err) ? (long int)bufsize : -1;
  // memcpy(msc_disk[lba] + offset, buffer, bufsize);
  // return bufsize;
}

long int onReadSD(uint32_t lba, uint32_t offset, void *buffer, uint32_t bufsize) {

  bool err = true;
  for (int i = 0; i < bufsize / SD_MMC.sectorSize(); i++) {
    err &= SD_MMC.readRAW((uint8_t *)buffer + (i * SD_MMC.sectorSize()), lba + i);
  }

  //DBG_OUTPUT_PORT.printf("MSC READ: lba: %u, offset: %u, bufsize: %u\n", lba,(unsigned int)offset, bufsize);
  return (err) ? (long int)bufsize : -1;
  // cardT.writeRAW(int *buffer, int sector);
