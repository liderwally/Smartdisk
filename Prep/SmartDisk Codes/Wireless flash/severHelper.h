...
void handleFileList(AsyncWebServerRequest *request) {
  ...
  if (!SD_MMC.exists(mainPath + "/")) { return replyServerError(request, FPSTR(FS_INIT_ERROR)); }

  if (!request->hasArg("comm1")) { return replyBadRequest(request, F("DIR ARG MISSING")); }

  String path = request->arg("comm1");
  if (path != mainPath + "/" && !SD_MMC.exists(path)) { return replyBadRequest(request, "BAD PATH"); }
...
void handleStatus(AsyncWebServerRequest *request) {
  ...
  if (SD_MMC.exists(mainPath + "/")) {
    ...
    json += SD_MMC.totalBytes();
    json += F("\", \"usedBytes\":\"");
    json += SD_MMC.usedBytes();
    json += "\"";
  } else {
    json += "\"false\"";
  }
...
String lastExistingParent(String path) {
  ...
  while (!path.isEmpty() && !SD_MMC.exists(path)) {
...
void handleFileCreate(AsyncWebServerRequest *request) {
  ...
  if (SD_MMC.exists(path)) { return replyBadRequest(request, F("PATH FILE EXISTS")); }
...
void handleNotFound(AsyncWebServerRequest *request) {
  ...
  if (SD_MMC.exists(mainPath + "/fileNotFound.txt")) {
    handleFileRead(request, &SD_MMC, mainPath + "/fileNotFound.txt", false);
    machineState = UNSOLVED;
  } else {
...
void handleIndexAdmin(AsyncWebServerRequest *request) {
  ...
  if (!request->authenticate(webUser, webPass)) {
    ...
    if (!SD_MMC.exists(mainPath + "/server/index.html")) {
      return replyNotFound(request, "index.html not found");
    }
    // Open the file for reading
    File file = SD_MMC.open(mainPath + "/server/index.html", "r");
    if (!file) {
